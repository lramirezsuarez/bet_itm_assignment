...
CREATE OR REPLACE VIEW GANADORES_SEMANALES AS
	SELECT
		BET_USERS.USER_NAME || '' || BET_USERS.MIDDLE_NAME || '' || BET_USERS.LAST_NAME || '' BET_USERS.SECOND_LAST_NAME AS NOMBRE_COMPLETO, SUM(BET_DETAILS.BET_AMOUNT)
	FROM
...
CREATE OR REPLACE VIEW DETALLES_APUESTAS AS
	...
	WHERE BETS_DETAILS.BET_TYPE LIKE 'SIMPLE' AND BETS_DETAILS.BET_ID = 123;
